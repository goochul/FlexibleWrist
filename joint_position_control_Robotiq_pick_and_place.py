...
def joint_position_control(robot_interface, controller_cfg):
    ...
    initial_positions = [-0.0868, 0.7511, -0.0658, -2.1055, -0.1331, 4.4531, 0.8282]

    approach_positions = [-0.0859, 0.9264, -0.0491, -1.7025, -0.1236, 4.2253, 0.8459]

    # For the unloading phase, simply reverse the loading positions.
    lift_positions = [-0.0854, 0.8785, -0.0503, -1.7287, -0.1275, 4.2039, 0.8496]

    target_positions = [-0.0957, 1.1665, -0.0278, -1.1444, -0.1451, 3.9082, 0.8787]

    # 1. Move to a "reset" or home position before starting (we use the first position from the loading phase)
    move_to_position(robot_interface, np.array(initial_positions), controller_cfg)
    if stop_movement.is_set():
        return
    time.sleep(0.5)
    ...
    move_to_position(robot_interface, np.array(approach_positions), controller_cfg)
    if stop_movement.is_set():
        return
    time.sleep(0.5)
    ...
    # 3. Lift the object
    move_to_position(robot_interface, np.array(lift_positions), controller_cfg)
    if stop_movement.is_set():
        return
    time.sleep(0.5)

    # 4. Move to a target position
    move_to_position(robot_interface, np.array(target_positions), controller_cfg)
    if stop_movement.is_set():
        return
    time.sleep(0.5)
