...
def read_force_sensor(force_sensor):
    while True:
        force = force_sensor.get_force_obs()
        print("Force: ", force)
        time.sleep(0.1)  # Adjust the sleep time as needed

# Main program continues running, potentially doing other tasks
...
def main():
    ...
    force_sensor = ForceSensor("/dev/ttyUSB0", np.array([-10.982798427581788, -144.29237174987793, 9.55335311985016]))
    force_sensor.force_sensor_setup()
    print("force sensor is set")
 #   force = force_sensor.get_force_obs()
 #   print("force: ", force)
    
    # Start a separate thread to continuously monitor the force sensor
#    force_thread = threading.Thread(target=check_force_limit, args=(force_sensor,))
#    force_thread.daemon = True
#    force_thread.start()

    # Create and start the thread
    force_thread = threading.Thread(target=read_force_sensor, args=(force_sensor,), name="ForceSensorThread")
    ...
    '''
    reset_joint_positions = [-0.7887920648673097, 0.5521591844079418, 0.3668758866257393, -2.608892855821465, -0.1520263171183264, 3.6705817207702824, 0.35279838085632825]
