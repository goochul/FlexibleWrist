...
def read_ft_sensor_data():
    ...
    while len(force_data) < max_samples and not stop_threads.is_set():
        ...
        force_data.append((elapsed_time, *force, *torque))  # Store both force and torque data
        time.sleep(0.01)
...
# Plot FT and Z-Position data in a single figure at the end
def plot_merged_data():
    fig, ax1 = plt.subplots()

    # Plot FT sensor data
    if force_data:
        times, fx, fy, fz, tx, ty, tz = zip(*force_data)
        ...
        ax1.plot(times, tx, label="Tx", linestyle='--', color='tab:blue')
        ax1.plot(times, ty, label="Ty", linestyle='--', color='tab:orange')
        ax1.plot(times, tz, label="Tz", linestyle='--', color='tab:green')
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Force/Torque (N/Nm)", color='tab:blue')
    ax1.set_xlim([0, 17.5])
    ax1.set_ylim([-20, 20])  # Set y-axis limits for force and torque values
    ...
    if timestamps:
        ...
        ax2.set_ylim([0.280, 0.360])  # Set y-axis limits for Z-position
        ax2.legend(loc="upper right")

    plt.title("FT Sensor Data and Z Position of End-Effector Over Time")
    plt.show()
...
def main():
    ...
    plot_merged_data()  # Display merged FT sensor and Z-position plot
