...
def process_dataset(file_path):
    ...
    # If touching point is found, calculate offset Y Position
    if touching_point_index is not None:
        touching_point_y_position = merged_data.loc[touching_point_index, 'Y Position']
        merged_data['Offset Y Position'] = merged_data['Y Position'] - touching_point_y_position
    else:
        # Initialize 'Offset Y Position' with NaN if no touching point is found
        merged_data['Offset Y Position'] = np.nan
...
aligned_z = [
    df.set_index('Timestamp')['Offset Y Position'].reindex(time).interpolate()
    for df in processed_datasets
]

# Reindex raw force and raw Y Position datasets to the common time index
aligned_raw_force = [
    df.set_index('Timestamp')['Force Magnitude'].reindex(time).interpolate()
    for df in processed_datasets
]
aligned_raw_z_position = [
    df.set_index('Timestamp')['Y Position'].reindex(time).interpolate()
    for df in processed_datasets
]
...
# Calculate the average touching time and average Y Position
...
# Calculate mean and standard deviation for raw force and Y Position
...
# First Plot: Filtered Force Magnitude and Offset Y Position
if valid_touching_times and valid_touching_z_positions:
    ...
    # Offset Y Position plot
    ax2 = ax1.twinx()
    ax2.plot(mean_z.index.to_numpy(), mean_z.to_numpy(), label="Offset Y Position Mean", color='red')
    ax2.fill_between(mean_z.index.to_numpy(), 
                    (mean_z - std_z).to_numpy(), 
                    (mean_z + std_z).to_numpy(), 
                    color='red', alpha=0.3, label="Y Position Std Dev")
    ax2.set_ylabel("Offset Y Position (m)", color='red')
...
if valid_touching_times and valid_touching_z_positions:
    ...
    # Calculate the standard deviation of Y Position at the buckling point
    ...
    ax1.set_title("Filtered Force Magnitude and Offset Y Position with Buckling Point - Robotiq W/O Screws")
...
#         z_position_60 = mean_z.loc[time_60]  # Get the Y Position at 60 seconds
#     else:
#         # Interpolate the Y Position if 60 seconds is not an exact index
...
#     # Mark the intersection point on the Y Position plot
...
#     # Add title and legend
#     ax1.set_title("Filtered Force Magnitude and Offset Y Position with Buckling Point and Intersection - Interpolation 2")
#     lines1, labels1 = ax1.get_legend_handles_labels()
#     lines2, labels2 = ax2.get_legend_handles_labels()
#     ax1.legend(lines1 + lines2, labels1 + labels2, loc='upper right')
# # ---------------- End of Buckling Point Detection ----------------

# Second Plot: Raw Force Magnitude and Non-Offset Y Position
ax3 = axs[1]
...
# Plot Non-Offset Y Position
ax4 = ax3.twinx()
ax4.plot(mean_raw_z_position_index, mean_raw_z_position_values, label="Non-Offset Y Position Mean", color="red", linestyle="--")
ax4.fill_between(mean_raw_z_position_index, 
                 mean_raw_z_position_values - std_raw_z_position_values, 
                 mean_raw_z_position_values + std_raw_z_position_values,
                 color="red", alpha=0.3, label="Y Position Std Dev")
ax4.set_ylabel("Y Position (m)", color="red")
ax4.tick_params(axis='y', labelcolor='red')
ax4.set_ylim([-0.04, 0.04])  # Adjusted for Y Position range
...
# Add title and legend
ax3.set_title("Raw Force Magnitude and Non-Offset Y Position with Average Touching Point")
