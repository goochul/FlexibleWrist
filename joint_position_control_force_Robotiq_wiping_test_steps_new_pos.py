...
torque_data = []
z_positions = []
eef_positions = []    # Global list to store end-effector (EEF) offsets over time
joint_positions = []
joint_velocities = []
timestamps = []
...
initial_eef_position = None  # This will store the starting EEF position
max_samples = 4000
video_duration = 150
pressing_time = 4
rs_camera_index = 6
...
def parse_args():
    ...
    # FT sensor toggle: default is disabled. Use --enable-ft-sensor to enable.
    ...
    # Camera toggle: default is disabled. Use --enable-camera to enable.
...
def return_to_initial_position(robot_interface, controller_cfg):
    ...
    reset_joint_positions = [-0.0089260, 0.3819599, -0.0253966,
                             -2.1973930, -0.0307321, 4.1700501, 0.7718912]
...
# Global variable for plotting events
event_markers = []
...
def move_to_position(robot_interface, target_positions, controller_cfg, event_label=None):
    """
    Moves the robot to the target joint positions.
    If the difference between the stored initial EEF position and the current state exceeds a threshold,
    update the starting configuration to the current state.
    """
    ...
    # In the control loop, update the starting EEF position if the error exceeds a threshold.
    # Here, we use 0.05 as the threshold.
    while True:
        ...
        if len(robot_interface._state_buffer) > 0:
            ...
            # If initial_eef_position is not set, initialize it with the current EEF position.
            if initial_eef_position is None:
                initial_eef_position = current_ee_position.copy()
            else:
                # Check the difference between current EEF and the stored initial position.
                diff = np.abs(np.array(current_ee_position) - np.array(initial_eef_position))
                if np.max(diff) > 0.002:
                    print("EEF error exceeds threshold; updating initial EEF position to current state.")
                    initial_eef_position = current_ee_position.copy()
            
            # Compute offset relative to the (possibly updated) initial EEF position.
...
def joint_position_control(robot_interface, controller_cfg):
    """
    Moves the robot through a sequence of discrete joint positions.
    The trajectory is divided into a loading phase and an unloading phase.
    """
    # Define discrete positions for the loading phase
    loading_positions = [
...
def plot_merged_data(data_folder):
    ...
    if force_data:
        times = [entry[0] for entry in force_data]
        Fx = [entry[1] for entry in force_data]
        Fy = [entry[2] for entry in force_data]
        Fz = [entry[3] for entry in force_data]
        force_magnitudes = [entry[4] for entry in force_data]
...
def main():
    global global_start_time, force_sensor, initial_eef_position
    ...
    initial_eef_position = None  # Initially, no starting EEF position is stored.
